...
async fn main() -> Result<()> {
    ...
    println!("   - Markers disclosed: {:?}", genetic_proof.markers_present);
    ...
    let verification_result = verify_genetic_proof_complete(
        &genetic_proof,
        &wallet_response,
        &requested_markers,
    ).await?;
    ...
    if verification_result {
        ...
        println!("\n🗑️  Session data will auto-expire in 10 minutes");
    ...
    let google_config = wallet_integration.generate_google_wallet_integration("did:proofzk:genetics_user456")?;
