...
async fn demo_local_encrypted_storage() -> Result<()> {
    ...
    println!(
        ...
        loaded_did.did == customer_did.did
    );
...
async fn demo_qr_code_transfer() -> Result<()> {
    ...
    println!(
        "   Same DID on both devices: {}",
        customer_did.did == phone_did.did
    );
...
async fn demo_browser_storage() -> Result<()> {
    ...
    println!("   Customer goes to airline website...");
...
async fn demo_airline_usage(customer_did: &DidIdentity) -> Result<()> {
    ...
    println!("   Airline knows customer is 18+ but not exact age");
...
pub fn show_customer_migration_path() {
    println!("\nCustomer Migration Path to Native Wallet Support:");
