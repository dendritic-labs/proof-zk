...
async fn test_did_web() -> Result<()> {
    ...
    println!(
        "  📄 Exportable DID document created ({} bytes)",
        did_json.len()
    );
...
async fn test_universal_resolver() -> Result<()> {
    ...
    if let Some(resolved_doc) = resolver.resolve(test_did).await? {
        println!(
            "  ✅ Resolved real DID:key cryptographically: {}",
            resolved_doc.id
        );
    } else {
...
async fn test_proofzk_with_real_dids() -> Result<()> {
    ...
    let wallet_credential = WalletCredential {
        ...
        public_key: passenger_identity.document.public_keys[0]
            .public_key_base58
            .clone(),
        metadata: {
    ...
    wallet_integration
        .register_wallet_credential(wallet_credential)
        .await?;
    ...
    println!(
        ...
        proof_presentation.to_string().len()
    );
